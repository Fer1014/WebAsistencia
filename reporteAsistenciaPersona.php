<?php
    ...
    $nombre_modulo = "Reporte Asistencia por persona";
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
            ...
            <div class="main-content">
                <section class="main">
                    <div class="samelineDiv">
                        <label>Alumno</label>
                        <input type="text" id="alumno" style="width:400px;max-width:100%;">
                    </div>
    ...
    <script type="text/javascript">
        ...
        document.addEventListener("DOMContentLoaded", function(e) {
            ...
            request.open('POST','api/selectAlumnos.php',true);
            request.onload=function(){
                ...
                if (request.status==200 && request.readyState==4) {
                    ...
                    var html='';
                    ar_alumnos=response.alumnos;
                    for (var i = 0; i < ar_alumnos.length; i++) {
                        var text=ar_alumnos[i]['nombre'];
                        html+=
                        '<div class="itemList" onclick="select_alumno('+ar_alumnos[i]['usuario_id']+',\''+text+'\')">'+ar_alumnos[i]['nombre']+'</div>';
                    }
        ...
        document.getElementById("curso").addEventListener("keyup",function(evt){
            ...
            for (var i = 0; i < ar_alumnos.length; i++) {
                var text=ar_alumnos[i]['nombre'];
                if (text.toUpperCase().indexOf(document.getElementById("alumno").value.toUpperCase())>=0) {
                    html+=
                    '<div class="itemList" onclick="select_alumno('+ar_alumnos[i]['usuario_id']+',\''+text+'\')">'+ar_alumnos[i]['nombre']+'</div>';
                }
            }
        ...
        function select_alumno(usuario_id,nombre){
            document.getElementById("step-2").style.display="block";
            usuario_id_g=usuario_id;
            document.getElementById("alumno").value=nombre;
            var fd=new FormData();
            fd.append('usuario_id',usuario_id);
            var request=new XMLHttpRequest();
            request.open('POST','api/ReporteAsistenciaAlumno.php',true);
            request.onload=function(){
                if (request.status==200 && request.readyState==4) {
                    let response=JSON.parse(request.responseText);
                    ...
                    var ar_body=[];
                    var html_body='';
                    var name1=ar_consulta2[0]['curso'];
                    var name_header=ar_consulta2[0]['curso'];
                    for (var i = 0; i < ar_consulta2.length; i++) {
                        ...
                        if (name_header==ar_consulta2[i]['curso']) {
                            html_header+=
                            '<th>'+format_fecha(ar_consulta2[i]['fecha'],ar_consulta2[i]['hora_inicio'])+'</th>';
                        }
                        if (name1==ar_consulta2[i]['curso']) {
                            html_body+='<td class="tdFecha '+ar_consulta2[i]['fecha']+'" style="color:'+color+';">'+asistencia+'</td>';
                        }else{
                            name1=ar_consulta2[i]['curso'];
        ...
        function format_fecha(fecha,hora){
            var fecha_for='';
            var ar_fecha=fecha.split("-");
            fecha_for=ar_fecha[2]+"/"+ar_fecha[1]+" "+hora[0]+hora[1]+":"+hora[2]+hora[3];
            return fecha_for;
        }
    </script>
</body>
